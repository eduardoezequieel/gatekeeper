...
export class LoginService {
  ...
  login(email: string, password: string): Observable<LoginResponse> {
    return this.http.post<LoginResponse>(environment.url + '/authentication/login', {
      "email": email,
      "password": password
    })
  }
  ...
  authWithRecoveryCode(code: string) {
    return this.http.post<LoginResponse>(environment.url + '/two-factor/recovery-account', {
      "email": this.user.value.email,
      "password": this.user.value.password,
      "recoveryCode": code
    })
  ...
  askPasswordRecovery(email: string) {
    return this.http.post(environment.url + '/authentication/request-password-reset', {
      "email": email
    })
  }
  ...
  refreshToken(token: string): Observable<RefreshTokenResponse> {
    return this.http.post<RefreshTokenResponse>(environment.url + '/authentication/refresh-token', {
      "refreshToken": token
    })
  }
